...
const storageKey = (sessionId: string) => `overmastery-pending-sets:${sessionId}`;
...
export function useOfflineSetQueue(sessionId: string, onRejected: (message: string) => void) {
  const [pending, setPending] = useState<Pending>({});
  const [, startTransition] = useTransition();
  const pendingRef = useRef<Pending>({});
  const flushing = useRef(false);

  const update = useCallback((next: Pending) => {
  ...
  const flush = useCallback(async () => {
    if (flushing.current || Object.keys(pendingRef.current).length === 0) return;
    flushing.current = true;
    try {
      const queue = Object.values(pendingRef.current).sort((left, right) => left.queuedAt - right.queuedAt);
      for (const item of queue) {
        let result;
        try {
          result = await completeSet({ loadKg: item.loadKg, loadUnit: item.loadUnit, reps: item.reps, sessionId: item.sessionId, setId: item.setId });
        } catch {
        ...
        if (!result.ok) onRejected(result.message ?? "A set saved offline couldn't be synced.");
      }
    } finally {
  ...
  const enqueue = useCallback((item: Omit<QueuedSet, "queuedAt">) => {
    update({ ...pendingRef.current, [item.setId]: { ...item, queuedAt: Date.now() } });
  }, [update]);

  const discard = useCallback((setId: string) => {
...
export function applyPendingSets(session: ActiveSession, pending: Record<string, QueuedSet>): ActiveSession {
    ...
    exercises: session.exercises.map((exercise) => ({
      ...
      sets: exercise.sets.map((set) => {
        ...
        return queued && set.status !== "completed"
          ? { ...set, enteredUnit: queued.loadUnit, loadKg: queued.loadKg, reps: queued.reps, status: "completed" as const }
          : set;
      }),
    })),
  };

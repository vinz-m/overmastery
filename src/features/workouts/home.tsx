...
export type HomeWorkoutExercise = { exerciseId: string; name: string; trackingType: TrackingType };
export type HomeWorkout = { exerciseCount: number; exercises: HomeWorkoutExercise[]; firstTarget?: string; id: string; lastTrainedAt?: string; name: string; preview: string[] };
...
export function WorkoutHome({ activeSession, archivedCatalog = [], catalog = [], displayName, email, featuredPrevious = {}, guidance, planningView = "workouts", timeZone, trainedAt = [], unitSystem = "metric", view = "home", workouts }: Props) {
  ...
  return <Today accountLabel={accountLabel} activeSession={activeSession} featuredPrevious={featuredPrevious} guidance={guidance} timeZone={timeZone} trainedAt={trainedAt} unitSystem={unitSystem} workouts={workouts} />;
}

// Enough to preview the session without turning the card into the full plan.
const previewLimit = 5;

function Today({ accountLabel, activeSession, featuredPrevious, guidance, timeZone, trainedAt, unitSystem, workouts }: { accountLabel: string; activeSession?: HomeActiveSession; featuredPrevious: Record<string, PreviousPerformance>; guidance: GuidanceState; timeZone: string; trainedAt: string[]; unitSystem: UnitSystem; workouts: HomeWorkout[] }) {
  ...
  return <main className={styles.tabContent}>
    ...
    {activeSession ? <section className={styles.resumeCard}>
      ...
      <Link href={`/sessions/${activeSession.id}`} transitionTypes={navForward}>Resume workout <CaretRightIcon aria-hidden="true" size={20} weight="bold" /></Link>
      ...
      <h2>{featured.name}</h2>
      {featured.exercises.length > 0 ? <ol aria-label="Exercises and your last result" className={styles.workoutPreview}>
        {featured.exercises.slice(0, previewLimit).map((exercise) => <li key={exercise.exerciseId}><span>{exercise.name}</span><small>{lastResult(exercise.trackingType, featuredPrevious[exercise.exerciseId], unitSystem)}</small></li>)}
        {featured.exercises.length > previewLimit && <li className={styles.previewMore}>+{featured.exercises.length - previewLimit} more</li>}
      </ol> : <p>{featured.firstTarget ?? `${featured.exerciseCount} exercises`}</p>}
      <StartWorkoutForm label="Start workout" showArrow workoutId={featured.id} />
      {workouts.length > 1 && <Link className={styles.alternateWorkout} href="/workouts" transitionTypes={navTab}>Choose another workout</Link>}
...
function TrainingWeek({ timeZone, trainedAt }: { timeZone: string; trainedAt: string[] }) {
  const week = trainingWeek(trainedAt, new Date(), timeZone);
  const summary = week.thisWeek === 0
    ? "No workouts yet this week"
    : `${week.thisWeek} ${week.thisWeek === 1 ? "workout" : "workouts"} this week`;
  return <section aria-labelledby="training-week-title" className={styles.week}>
    <header><h2 id="training-week-title">This week</h2><Link href="/progress" transitionTypes={navTab}>View progress</Link></header>
    <div className={styles.weekCard}>
      <ol className={styles.weekDays}>
        {week.days.map((day) => <li className={[day.trained && styles.weekTrained, day.isToday && styles.weekToday, day.isFuture && styles.weekFuture].filter(Boolean).join(" ")} key={day.key}>
          <span aria-hidden="true">{day.label}</span><i aria-hidden="true" />
          <span className={styles.visuallyHidden}>{weekdayName(day.key)}{day.isToday ? " (today)" : ""}: {day.trained ? "trained" : "no workout"}</span>
        </li>)}
      </ol>
      <p>{summary}<span> · {week.lastWeek} last week</span></p>
    </div>
  </section>;
}
...
function lastResult(trackingType: TrackingType, previous: PreviousPerformance | undefined, unitSystem: UnitSystem) {
...
function WorkoutLibrary({ activeSession, workouts }: { activeSession?: HomeActiveSession; workouts: HomeWorkout[] }) {
  return <>
    <header className={styles.libraryHeader}><div><p>Library</p><h1>Workouts</h1><span>{workouts.length} saved {workouts.length === 1 ? "workout" : "workouts"}</span></div><Link href="/workouts/new" transitionTypes={navForward}>New workout</Link></header>
    {activeSession && <Link className={styles.activeBanner} href={`/sessions/${activeSession.id}`} transitionTypes={navForward}><span>Workout in progress</span><strong>Resume {activeSession.templateName}</strong><small>Finish or discard it to start another workout.</small><i><CaretRightIcon aria-hidden="true" size={22} weight="bold" /></i></Link>}
    {workouts.length ? <section className={styles.workoutList}>{workouts.map((workout) => <article className={styles.workoutRow} key={workout.id}>
      ...
      <div className={styles.workoutActions}>
        <Link aria-label={`Edit ${workout.name}`} href={`/workouts/${workout.id}/edit`} transitionTypes={navForward}>Edit plan</Link>
        <StartWorkoutForm disabled={Boolean(activeSession)} label="Start" workoutId={workout.id} />
      </div>
    </article>)}</section> : <section className={styles.emptyState}><h2>No workouts yet</h2><p>Create a reusable workout from the exercise library.</p><Link className={styles.primaryLink} href="/workouts/new" transitionTypes={navForward}>Create workout <CaretRightIcon aria-hidden="true" size={20} weight="bold" /></Link></section>}
  </>;
}
